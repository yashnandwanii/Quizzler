...
class _EnhancedResultsScreenState extends State<EnhancedResultsScreen> {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInfoRow('Difficulty', widget.preferences.difficulty, Icons.trending_up),
          _buildInfoRow('Questions', '${widget.totalQuestions}', Icons.quiz),
          _buildInfoRow('Time Spent', '${(widget.timeSpent / 60).toStringAsFixed(1)} min', Icons.timer),
          if (widget.preferences.tags.isNotEmpty)
            _buildInfoRow('Topics', widget.preferences.tags.join(', '), Icons.label),
        ],
      ),
    );
  }
  ...
  Future<void> _saveQuizResultAndUpdateStats() async {
    ...
    try {
      ...
      // Save quiz history
      await QuizHistoryService.addQuizToHistory(
        userId: userId,
